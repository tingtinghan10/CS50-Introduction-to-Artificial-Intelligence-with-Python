...
class NimAI():
    ...
    def best_future_reward(self, state):
        ...
        for action in available_actions:
            if self.get_q_value(state, action) > q_score:
                q_score = self.q[(tuple(state), action)]       
        return q_score
    ...
    def choose_action(self, state, epsilon=True):
        ...
        # if no best action, return a random available action
        if max_reward == 0:
        ...
        if epsilon == False:
            ...
            action = random.choices(('random', 'best'), weights=(self.epsilon, 1 - self.epsilon))
            if action == 'random':
                return random.choice(tuple(available_actions))
            else:
                return best_action
